/* Bantam Java Compiler and Language Toolset.
   ...
   Edited by: Ana Sofia Solis Canales, He He, Josh Hews, Erin Lavoie
   Class: CS461
   Project: 3
   Date: 3/12/17
...
public class SemanticAnalyzer {
    ...
    public ClassTreeNode analyze() {
        ...
        //record reservedWords
        List<String> reservedWords = Arrays.asList("int", "boolean", "super", "this", "void", "null");

        //Build the other Class Tree Nodes
        ClassTreeNodeBuilderVisitor ctnbV = new ClassTreeNodeBuilderVisitor(this.classMap,
                                                                            this.errorHandler,
                                                                            new HashSet<>(reservedWords));
        ctnbV.buildClasses(this.program);


        //Connect Tree
        ClassConnectorVisitor ccv = new ClassConnectorVisitor(this.classMap,
                                                            this.errorHandler,
                                                            new HashSet<>(reservedWords));

        ccv.connectClasses(this.program);


        //Check for Cyclic Relationships in the Tree
        LoopChecker loop = new LoopChecker(this.classMap, this.errorHandler);
        loop.checkLoop();

        //If there is a cyclic error, stop compiling, report to the user.
        errorHandler.checkErrors();

        //Build the environment for all classes in Tree
        EnvironmentBuilderVisitor environmentBuilder = new EnvironmentBuilderVisitor(this.classMap,
                                                                                    this.errorHandler,
                                                                                    new HashSet<>(reservedWords));
        environmentBuilder.buildEnvironment(this.program);

        //check for main class with main method
        MainChecker mainChecker = new MainChecker(this.classMap, this.errorHandler);
        mainChecker.hasMainMethodInMainClass();

        //Type Check the Tree
        TypeCheckerVisitor typeCheckerVisitor = new TypeCheckerVisitor(this.classMap,
                this.errorHandler, new HashSet<> (reservedWords));
        typeCheckerVisitor.checkType(this.program);

        //Check Break Stmt to be correctly formate
        BreakStmtCheckerVisitor breakStmtCheckerVisitor = new BreakStmtCheckerVisitor(this.classMap,
                                                                                    this.errorHandler);
        breakStmtCheckerVisitor.checkIllegalBreaks(this.program);


        //checks for reported errors
        errorHandler.checkErrors();
    ...
    private void updateBuiltins() {
        ...
        // Connect the builtins to Object
        ClassTreeNode object = classMap.get("Object");
        object.addChild(classMap.get("Sys"));
        object.addChild(classMap.get("TextIO"));
        object.addChild(classMap.get("String"));
