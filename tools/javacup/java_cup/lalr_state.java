 ...
 *  a potential configuration of the bantam.parser.  If the item set of a state
 *  includes an item such as: <pre>
 *    [A ::= B * C d E , {a,b,c}]
 *  </pre> 
 *  this indicates that when the bantam.parser is in this state it is currently
 ...
 *  the bantam.parser is normally looking for several things at once (represented
 ...
 *
 *  At runtime, the bantam.parser uses a viable prefix recognition machine made up
 *  of these states to parse.  The bantam.parser has two operations, shift and reduce.
 ...
 *  the end of the transition).  For a reduce operation, the bantam.parser is
 ...
 *  bantam.parser first attempted to find the RHS.  From this state it makes a
...
public class lalr_state {
   ...
   *  the bantam.parser was now looking for.  In this case we would items
